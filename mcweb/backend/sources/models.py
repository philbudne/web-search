...
class Source(models.Model):
    ...
    def _scrape_source(source_id: int, homepage: str, name: str, verbosity: int = 1) -> str:
        ...
        if total == 0:          # nothing seen now?
            if old == 0:        # had nothing before?
                msg2 = " (no existing feeds)"
            else:
                msg2 = ""
            add_line(f"no feeds discovered{msg2}\n")
        elif added == 0:        # added nothing?
            if verbosity == 0:  # didn't output per-feed msgs?
                msg2 = f" ({confirmed} existing feed(s) rediscovered)"
            else:
                msg2 = ""
            add_line(f"no new feeds discovered{msg2}\n")
