...
def _featured_collections(platform: str | None): # returns queryset
    """
    ...
    # lower number rank comes first (0th, 1st, 2nd, ...)
    # secondary sort by id so ties deterministic
    queryset = queryset.filter(featured_rank__isnull=False)\
...
def _cached_serialized_featured_collections(platform) -> str:
    ...
    uses ALL arguments to form cache key, including positionals, which
    includes "self", which means it's always different, and caching
    never happens.
    """
    featured_collections = _featured_collections(platform)
    serializer = CollectionViewSet.serializer_class(featured_collections, many=True)
