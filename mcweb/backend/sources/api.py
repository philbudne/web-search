...
def _featured_collections(platform) -> list[Collection]:
    ...
    """
    featured_collections = list(CollectionViewSet.queryset.filter(platform=platform, featured=True))
    ...
    if pos_map:
        ...
        featured_collections.sort(key=lambda c: pos_map.get(c.id, 54321))
