...
class IndexedIContains(Lookup):
    """
    An __contains lookup operation that generates ILIKE to use trigram index
    """
    ...
    def get_queryset(self):
        ...
        if name is not None:
            if constance.config.SRCS_KW_NEWEST_SEARCH:
                ...
                # can run under 50 millseconds, while queries NOT using the index run 5
                # seconds (ie; two orders of magnitude), and could support a
                # find-as-you-type (live/incremental) search!!
                ...
                # When attempting to OR in alt_query along with name and label queries PG
                ...
                # (you want to see "sources_source_name_label_trigram_gin_index")
